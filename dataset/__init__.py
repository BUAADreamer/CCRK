...
from dataset.pretrain_dataset_multilingual import ImageMultiTextDataset, RegionMultiTextDataset, ImageMonoTextDataset, \
    ParaTextDataset
...
def create_dataset(dataset, config):
    ...
    if dataset == 'pretrain_multilingual':
        ...
        if len(config['train_file_mono']):
            ...
            mono_dataset = ImageMonoTextDataset(config, config['train_file_mono'],
                                                rank=int(os.environ.get('RANK') or 0),
                                                world_size=int(os.environ.get('WORLD_SIZE') or 1), shuffle=True,
                                                repeat=True, transform=pretrain_transform)
        else:
        ...
        for k, rpath in config['test_file'].items():
            if 'mscoco/en.test.json' in rpath or 'mscoco/ja.test.json' in rpath:
                for i in range(5):
                    test_dataset_dict[f'{k}{i}'] = re_eval_dataset(rpath, test_transform, config['image_root'], fold=i)
            else:
