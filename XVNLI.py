...
def train(model, data_loader, optimizer, tokenizer, epoch, device, scheduler, config):
    ...
    for i, (image, text, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        text_inputs = tokenizer(text, padding='longest', max_length=config['max_tokens'], truncation=True,
    ...
    if args.evaluate:
        ...
        if utils.is_main_process():
            ...
            if args.output_hdfs and not args.fewshot:
                os.system(
                    f"hdfs dfs -put {os.path.join(args.output_dir, 'model_state_epoch_{}.th'.format(best_epoch))} {args.output_hdfs}")
                os.system(
                    f"hdfs dfs -put {os.path.join(args.output_dir, 'model_state_epoch_{}.th'.format(max_epoch - 1))} {args.output_hdfs}")
                os.system(f"hdfs dfs -put {os.path.join(args.output_dir, 'log.txt')} {args.output_hdfs}")
                os.system(
                    f"hdfs dfs -put {os.path.join(args.output_dir, 'eval_result_{}.txt'.format(best_epoch))} {args.output_hdfs}")
                os.system(
                    f"hdfs dfs -put {os.path.join(args.output_dir, 'eval_result_{}.txt'.format(max_epoch))} {args.output_hdfs}")
