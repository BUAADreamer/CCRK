...
def build_text_encoder(config, vision_width, load_text_params=False, use_mlm_loss=False, config_text=None):
    ...
    if use_mlm_loss:  # for pre-training, load_text_params by default (otherwise notimplemented)
        ...
        if 'xlm-roberta-large' in config[
            'text_encoder']:  # i need it to encode both image-caption pairs and parallel text pairs
...
class XVLMBase(nn.Module):
    def __init__(self, config=None, load_vision_params=False, load_text_params=False,
        ...
        self.sample_lan_file = config.get('sample_lan_file', '')
        self.neg_sample_type = config.get('neg_sample_type', 1)
        self.need_divm = config.get('need_divm', 0)
        self.itc_allgather = config.get('itc_allgather', 0)
        self.vicreg = config.get('vicreg', 1)
    ...
    def get_unialign_contrastive_loss(self, image_feat, text_feat, idx=None):
        assert image_feat.size(-1) == self.embed_dim
        assert text_feat.size(-1) == self.embed_dim
        if self.itc_allgather == 0:
            image_feat_all = allgather(image_feat, torch.distributed.get_rank(), torch.distributed.get_world_size())
            text_feat_all = allgather(text_feat, torch.distributed.get_rank(), torch.distributed.get_world_size())
        else:
        ...
        bsz = image_feat_all.shape[0]
        assert text_feat_all.shape[0] % bsz == 0
        tsz = text_feat_all.shape[0] // bsz
        logits = image_feat_all @ text_feat_all.t() / self.temp
        if idx is None:
            labels_i2t = torch.zeros((bsz, tsz * bsz), device=image_feat.device, dtype=torch.float32)
            for i in range(bsz):
                labels_i2t[i][tsz * i:tsz * i + tsz] = 1
            labels_t2i = labels_i2t.t()
            # labels_i2t = labels_i2t.softmax(dim=1)
            # labels_t2i = labels_t2i.softmax(dim=1)
            loss_i2t = -torch.sum(F.log_softmax(logits, dim=1) * labels_i2t, dim=1).mean()
            loss_t2i = -torch.sum(F.log_softmax(logits.t(), dim=1) * labels_t2i, dim=1).mean()
            # loss_i2t = F.cross_entropy(logits, labels_i2t)
            # loss_t2i = F.cross_entropy(logits.t(), labels_t2i)
        else:
            idx = idx.view(-1, 1)
            assert idx.size(0) == image_feat.size(0)
            idx_all = allgather(idx, torch.distributed.get_rank(), torch.distributed.get_world_size())
            ...
            loss_i2t = -torch.sum(F.log_softmax(logits, dim=1) * labels, dim=1).mean()
            loss_t2i = -torch.sum(F.log_softmax(logits.t(), dim=1) * labels, dim=1).mean()
        res_loss = (loss_i2t + loss_t2i) / 2
        if self.need_divm == 0:
            res_loss = res_loss / tsz
        elif self.need_divm > 0:
            res_loss = res_loss / self.need_divm
        return res_loss
    ...
    def get_unialign_matching_loss(self, image_embeds, image_atts, image_feat, text_embeds, text_atts, text_feat,
                                   idx=None):
        """
        Matching Loss with hard negatives
        calculate ttm and itm loss
        """
        text_embeds_pos = []
        text_atts_pos = []
        text_pos_idx_ls = []
        bs = image_embeds.size(0)
        tsz = text_embeds.size(0) // bs  # 语言个数
        with torch.no_grad():
            sim_i2t = image_feat @ text_feat.t() / self.temp
            sim_t2i = text_feat @ image_feat.t() / self.temp
            # max_sim = torch.max(sim_i2t) + 1
            weights_i2t = F.softmax(sim_i2t, dim=1) + 1e-5
            weights_t2i = F.softmax(sim_t2i, dim=1) + 1e-5
            max_weight = torch.max(weights_i2t) + 1e-5
            max_sim = torch.max(sim_i2t) + 1e-5
            # weights_i2t = sim_i2t + 2 / self.temp
            # weights_t2i = sim_t2i + 2 / self.temp
            statistic_str = ''
            random_slice = list(range(tsz))

        for b in range(bs):
            pos_idx = 0
            if self.neg_sample_type == 0:
                pos_idx = random.choice(random_slice)
            elif self.neg_sample_type == 1:
                w_slice = max_weight - weights_i2t[b, b * tsz:b * tsz + tsz]
                pos_idx = torch.multinomial(w_slice, 1).item()
            elif self.neg_sample_type == 2:
                w_slice = max_sim - sim_i2t[b, b * tsz:b * tsz + tsz]
                pos_idx = torch.multinomial(w_slice, 1).item()
            statistic_str += str(pos_idx) + ' '
            pos_idx = b * tsz + pos_idx
            text_embeds_pos.append(text_embeds[pos_idx])
            text_atts_pos.append(text_atts[pos_idx])
            text_pos_idx_ls.append(pos_idx)
        text_embeds_pos = torch.stack(text_embeds_pos, dim=0)
        text_atts_pos = torch.stack(text_atts_pos, dim=0)

        with torch.no_grad():
            if self.sample_lan_file:
                with open(self.sample_lan_file, 'a') as f:
                    f.write(statistic_str + '\n')
            if idx is None:
                # weights_i2t.fill_diagonal_(0)
                # weights_t2i.fill_diagonal_(0)
                for i in range(bs):
                    weights_i2t[i, tsz * i:tsz * i + tsz] = 0
                    weights_t2i[tsz * i:tsz * i + tsz, i] = 0
            else:
                idx = idx.view(-1, 1)
                assert idx.size(0) == bs
                mask = torch.eq(idx, idx.t())
        ...
        image_atts_neg = []
        for b in range(bs):
            neg_idx = torch.multinomial(weights_t2i[text_pos_idx_ls[b]], 1).item()
            image_embeds_neg.append(image_embeds[neg_idx])
            image_atts_neg.append(image_atts[neg_idx])

        image_embeds_neg = torch.stack(image_embeds_neg, dim=0)  # [bs,m]
        image_atts_neg = torch.stack(image_atts_neg, dim=0)
        ...
        text_atts_neg = []
        for b in range(bs):
            neg_idx = torch.multinomial(weights_i2t[b], 1).item()  # [bs,m]
            text_embeds_neg.append(text_embeds[neg_idx])
            text_atts_neg.append(text_atts[neg_idx])

        text_embeds_neg = torch.stack(text_embeds_neg, dim=0)
        text_atts_neg = torch.stack(text_atts_neg, dim=0)
        text_embeds_all = torch.cat([text_embeds_pos, text_embeds_neg], dim=0)
        text_atts_all = torch.cat([text_atts_pos, text_atts_neg], dim=0)
        image_embeds_all = torch.cat([image_embeds_neg, image_embeds], dim=0)
        image_atts_all = torch.cat([image_atts_neg, image_atts], dim=0)

        cross_pos = self.get_cross_embeds(image_embeds, image_atts, text_embeds=text_embeds_pos,
                                          text_atts=text_atts_pos)[:, 0, :]
        cross_neg = self.get_cross_embeds(image_embeds_all, image_atts_all, text_embeds=text_embeds_all,
                                          text_atts=text_atts_all)[:, 0, :]

        output = self.itm_head(torch.cat([cross_pos, cross_neg], dim=0))  # [bs*3]
        itm_labels = torch.cat([torch.ones(bs, dtype=torch.long),
                                torch.zeros(2 * bs, dtype=torch.long)], dim=0).to(image_embeds.device)  # [bs*3,2]
        return F.cross_entropy(output, itm_labels), text_pos_idx_ls
    ...
    def get_unialign_mlm_loss(self, text_ids_masked, text_atts, image_embeds, image_atts, masked_pos, masked_ids,
                              text_pos_idx_ls):
        text_ids_masked_pos = []
        text_atts_pos = []
        masked_pos_pos = []
        masked_ids_pos = []
        bs = image_embeds.size(0)
        tsz = text_atts.size(0)
        for b in range(bs):
            i = text_pos_idx_ls[b]
            text_ids_masked_pos.append(text_ids_masked[i])
            text_atts_pos.append(text_atts[i])
            masked_pos_pos.append(masked_pos[i])
            masked_ids_pos.append(masked_ids[i])
        text_ids_masked_pos = torch.stack(text_ids_masked_pos)
        text_atts_pos = torch.stack(text_atts_pos)
        masked_pos_pos = torch.stack(masked_pos_pos)
        masked_ids_pos = torch.stack(masked_ids_pos)
        return self.text_encoder(text_ids_masked_pos,
    ...
    def get_pair_invariance_loss(self, feat1, feat2):
        return F.mse_loss(feat1, feat2)

    def get_invariance_loss(self, image_feat, text_feat):
        n, d = image_feat.shape
        m = text_feat.shape[0] // n
        invariance_loss = 0
        for i in range(m):
            pos_ls = [(m * j + i) for j in range(n)]
            invariance_loss += self.get_pair_invariance_loss(image_feat, text_feat[pos_ls])
        invariance_loss /= m
        return invariance_loss

    def get_single_variance_loss(self, feat):
        std_feat = torch.sqrt(torch.var(feat, dim=0) + 1e-4)
        std_loss = torch.mean(F.relu(1 - std_feat))
        return std_loss

    def get_variance_loss(self, image_feat, text_feat):
        return self.get_single_variance_loss(image_feat) + self.get_single_variance_loss(text_feat)

    def get_single_covariance_loss(self, feat):
        n, d = feat.shape
        feat = feat - torch.mean(feat, dim=0)
        cov_feat = (feat.T @ feat) / (n - 1)
        cov_feat.fill_diagonal_(0)
        cov_loss = cov_feat.pow(2).sum() / d
        return cov_loss

    def get_covariance_loss(self, image_feat, text_feat):
        return self.get_single_covariance_loss(image_feat) + self.get_single_covariance_loss(text_feat)

    def get_vicreg_loss(self, image_feat, text_feat):
        a, b, c = 25, 25, 1
        invar_loss = self.get_invariance_loss(image_feat, text_feat)
        vari_loss = self.get_variance_loss(image_feat, text_feat)
        cov_loss = self.get_covariance_loss(image_feat, text_feat)
        return a * invar_loss + b * vari_loss + c * cov_loss
